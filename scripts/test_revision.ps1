...
if ($Mode -eq 'Quick') {
    $Steps = @(
        @{ Name='candidate-unit-tests'; File=$Gradle; Args=@(':server:core:test','--tests','dev.monaka.tracking.revision.*','--no-daemon') }
    )
}
else {
    $Steps = @(
        @{ Name='software-regression'; File=$Gradle; Args=@(':server:core:test',':server:desktop:test',':server:desktop:shadowJar','--no-daemon') }
    )
}
